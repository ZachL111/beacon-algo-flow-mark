...
`beacon-algo-flow-mark` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for flow analysis with seeded input scenarios, deterministic summary checks, and documented operating limits.
...
The score is deterministic. The current threshold is 180. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 2.
